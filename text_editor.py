...
def openFile(window, text_edit):
    filepath = askopenfilename(filetypes=[("Text Files", "*.txt"), ("Python Files", "*.py"), ("C Files", "*.c"), ("C++ Files", "*.cpp")])
...
def saveFile(window, text_edit):
    filepath = asksaveasfilename(filetypes=[("Text Files", "*.txt"), ("Python Files", "*.py"), ("C Files", "*.c"), ("C++ Files", "*.cpp")])
...
def main():
    ...
    text_edit = tk.Text(window, font="Calibri 18")
    text_edit.grid(row = 0, column=1)

    frame = tk.Frame(window, relief=tk.RAISED, bd=2)
    save_button = tk.Button(frame, text="Save", command=lambda : saveFile(window, text_edit))
    open_button = tk.Button(frame, text="Open", command=lambda : openFile(window, text_edit))
